...
import {FindByIdTodoQueryHandler} from "../../../../src/application/todo/find/find-by-id-todo.query.handler";
import {FindByIdTodoQuery} from "../../../../src/application/todo/find/find-by-id-todo.query";

describe('Find By Id Todo Query Handler', () => {
  ...
  test('find by id and call transformer', async () => {
    ...
    const handler = new FindByIdTodoQueryHandler(todoByIdFinder, todoDataTransformer)
    const result = await handler.invoke(
      new FindByIdTodoQuery('1')
    )
    expect(result).toEqual(transformerResult)
  })

  test('find by id and throw exception', async () => {
    todoByIdFinder.invoke.mockReturnValue(
      new Promise((resolve) => {
        resolve(undefined)
      })
